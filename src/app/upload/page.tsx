...
export default function UploadPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setStep("uploading");
    try {
      const { secure_url, public_id, thumbnail_url } = await uploadToCloudinary(file, setProgress);
    ...
    return (
      <div className="min-h-dvh bg-[#0d0d16] flex flex-col items-center justify-center px-8 text-center">
        ...
        <div className="flex flex-col gap-3 w-full">
          <button onClick={() => router.push("/profile")} className="w-full bg-[#7c3aed] text-white font-bold py-3.5 rounded-2xl">View my videos</button>
