...
export default function MapPage() {
  ...
  // User location
  const { coords, permissionState, requesting, requestLocation } =
    useGeolocation();

  // Places
  const { places, fetchByBounds } = usePlaces({
    category,
  });

  // Prevent opening same place multiple times
      ...
      <div className="absolute inset-0 z-0">
        {!isLoaded && !loadError && (
          <div className="w-full h-full flex flex-col items-center justify-center bg-[#0a0a0f]">
            ...
            <p className="text-sm text-white/40 font-semibold tracking-wide">
              LOADING MAPS...
            </p>
          </div>
        )}
