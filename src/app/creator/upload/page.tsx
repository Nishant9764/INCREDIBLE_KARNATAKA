...
import { CATEGORIES, ALLOWED_VIDEO_TYPES, MAX_VIDEO_SIZE } from "@/constants";
import { cn } from "@/lib/utils";
import PlaceSearch from "@/features/upload/PlaceSearch";
import PlaceCreateModal from "@/features/upload/PlaceCreateModal";
import type { IPlace } from "@/types";

type Step = "pick" | "details" | "uploading" | "done";

const inputCls =
  "w-full bg-[#0d0d16] border border-[#2a2a3e] rounded-xl px-4 py-3 text-sm text-white placeholder-[#555577] focus:border-[#7c3aed] focus:ring-1 focus:ring-[#7c3aed]/30 transition-all";
...
export default function UploadPage() {
  ...
  const handleFile = (f: File) => {
    ...
    if (!ALLOWED_VIDEO_TYPES.includes(f.type)) {
      setError("Only MP4, MOV, or WEBM files allowed");
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedPlace && !newPlace.name.trim()) {
      setError("Please select or create a place.");
      return;
    }
    ...
    try {
      ...
      const { downloadURL, publicId } = await uploadVideo(
        file,
        path,
        setProgress
      );
      ...
      const res = await fetch("/api/videos", {
        ...
        body: JSON.stringify({
          ...
          placeId:
            selectedPlace?._id === "__NEW_PLACE__"
              ? undefined
              : selectedPlace?._id,
  ...
  if (step === "done") {
    return (
      <div className="min-h-dvh bg-[#0d0d16] flex flex-col items-center justify-center px-8 text-center">
        ...
        <p className="text-xs text-[#555577] mb-8">
          It appears in the feed once approved by our team.
        </p>
        <div className="flex flex-col gap-3 w-full">
          <button
            onClick={() => router.push("/creator/content")}
            className="w-full bg-[#7c3aed] text-white font-bold py-3.5 rounded-2xl"
          >
            View my videos
          </button>
          <button
            onClick={reset}
            className="w-full bg-[#161622] border border-[#2a2a3e] text-white font-semibold py-3.5 rounded-2xl"
          >
            Upload another
          </button>
        </div>
  ...
  if (step === "uploading") {
    return (
      <div className="min-h-dvh bg-[#0d0d16] flex flex-col items-center justify-center px-8 text-center">
        ...
        <h2 className="text-xl font-bold text-white mb-1">Uploading</h2>
    ...
    return (
      <div className="min-h-dvh bg-[#0d0d16] flex flex-col">
        <div className="flex items-center gap-3 px-4 pt-14 pb-5">
          <button
            onClick={() => router.back()}
            className="w-9 h-9 rounded-xl bg-[#161622] flex items-center justify-center"
          >
        ...
        {error && (
          <div className="mx-4 mb-4 bg-rose-500/10 border border-rose-500/30 rounded-xl px-4 py-3 text-sm text-rose-400">
            {error}
          </div>
        )}
          ...
          <div
            ...
            className={cn(
              ...
              dragOver
                ...
                : "border-[#2a2a3e] bg-[#161622]/50 hover:border-[#7c3aed]/50"
            )}
      ...
      <form onSubmit={handleSubmit} className="px-4 py-4 space-y-4 pb-32">
        ...
        <Field label="Place *">
          <PlaceSearch
            ...
            onCreateNew={() => setShowCreatePlace(true)}
          />
        </Field>

        <PlaceCreateModal
          open={showCreatePlace}
          place={newPlace}
        ...
        <Field label="Tags (comma separated)">
          <div className="relative">
            ...
            <input
              ...
              placeholder="monsoon, hidden gem, waterfall"
        ...
        <div className="bg-amber-500/8 border border-amber-500/15 rounded-xl p-3.5 flex gap-2.5">
          ...
          <p className="text-xs text-amber-300/90 leading-relaxed">
            Your video will be reviewed before appearing in the feed. Usually
            24–48 hours.
          </p>
        </div>
        <button
          type="submit"
          className="w-full bg-[#7c3aed] hover:bg-[#6d28d9] active:scale-[0.98] text-white font-bold py-4 rounded-2xl transition-all flex items-center justify-center gap-2"
        >
...
function Field({
  ...
  return (
    <div>
      <label className="text-xs font-semibold text-[#555577] mb-1.5 block">
        {label}
      </label>
      {children}
    </div>
