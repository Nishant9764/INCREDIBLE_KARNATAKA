...
export function VideoCard({
  ...
  const creator =
    typeof video.creatorId === "object"
      ...
      : null;
  const place = typeof video.placeId === "object" ? video.placeId : null;
  ...
  const distKm =
    video.distanceKm ??
    (() => {
      if (!userLocation || !place?.location?.coordinates) return null;
      const R = 6371;
      const [, lat] = place.location.coordinates;
      const [lng] = place.location.coordinates;
  ...
  const travelMins = distKm
    ? Math.round((distKm / travelMode.speedKmh) * 60)
    : null;
        ...
        <div className="absolute right-3 bottom-36 flex flex-col items-center gap-4 z-10">
          ...
          >
            <Bookmark
              className={cn(
                "w-[22px] h-[22px]",
                saved ? "text-[#a78bfa] fill-[#a78bfa]" : "text-white"
              )}
            />
          </RailBtn>
                ...
                {travelMins && (
                  <>
                    ...
                    <span className="text-white/70">
                      {travelMode.icon} {fmtTime(travelMins)}
                    </span>
                  </>
                )}
              </button>
